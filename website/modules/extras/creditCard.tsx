...
const CreditCardForm = ({handlePaymentClick} : {handlePaymentClick:any}) => {
  ...
  const handleCardNameChange = (event: { target: { value: any; }; }) => {
    setCardName(event.target.value);
  };
  const handleCardDateChange = (event: { target: { value: any; }; }) => {
    setCardDate(event.target.value);
  };
  const handleCardSecurityChange = (event: { target: { value: any; }; }) => {
    setCardSecurity(event.target.value);
  };
    ...
    <div className="flex min-h-screen flex-col items-center justify-between p-6 lg:p-24">
      <form className="bg-white w-full max-w-3xl mx-auto px-4 lg:px-6 py-8 shadow-md rounded-md flex flex-col lg:flex-row">
        <div className="w-full lg:w-1/2 lg:pr-8 lg:border-r-2 lg:border-slate-300">
          <div className="mb-4">
            <label htmlFor="cardNumber" className="text-neutral-800 font-bold text-sm mb-2 block">Numero de carte</label>
            <input
              id="cardNumber"
              type="number"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="mb-4">
              <label htmlFor="cardHolder" className="text-neutral-800 font-bold text-sm mb-2 block">Nom</label>
              <input
                id="cardHolder"
                type="text"
                className="flex h-10 w-full rounded-md border-2 bg-background px-4 py-1.5 text-lg ring-offset-background placeholder-text-muted-foreground focus-visible:outline-none focus-visible:border-purple-600 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 undefined"
            ...
            <div className="mb-4">
              <label htmlFor="expiryDate" className="text-neutral-800 font-bold text-sm mb-2 block">Date d&apos;expiration</label>
              <input
                id="expiryDate"
                type="text"
                className="flex h-10 w-full rounded-md border-2 bg-background px-4 py-1.5 text-lg ring-offset-background placeholder-text-muted-foreground focus-visible:outline-none focus-visible:border-purple-600 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 undefined"
            ...
            <div className="mb-4 lg:col-span-1">
              <label htmlFor="cvv" className="text-neutral-800 font-bold text-sm mb-2 block">CVV</label>
              <input
                id="cvv"
                type="text"
                className="flex h-10 w-full rounded-md border-2 bg-background px-4 py-1.5 text-lg ring-offset-background placeholder-text-muted-foreground focus-visible:outline-none focus-visible:border-purple-600 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 undefined"
        ...
        <div className="w-full lg:w-1/2 lg:pl-8">
          <div className="w-full max-w-sm h-56" style={{ perspective: "1000px" }}>
            <div id="creditCard"
                 className={`relative crediCard cursor-pointer transition-transform duration-500 transform-gpu ${showBackCard ? '[transform:rotateY(180deg)]' : ''}`}
                 style={{ transformStyle: "preserve-3d" }} onClick={toggleCard}>
              <div className="w-full h-56 m-auto rounded-xl text-white shadow-2xl absolute"
                ...
                <div className="w-full px-8 absolute top-8">
                  ...
                  <div className="pt-1">
                    <p className="font-light">Numero de carte</p>
                    <p
                      id="imageCardNumber"
                      className="font-medium tracking-more-wider h-6"
                    >
                      {cardNumber ? cardNumber : "4256 4256 4256 4256"}
                    </p>
                  </div>
                  <div className="pt-6 flex justify-between">
                    <div>
                      <p className="font-light">Nom</p>
                      <p
                        id="imageCardName"
                        className="font-medium tracking-widest h-6"
                      >
                        {cardName ? cardName : "John Doe"}
                      </p>
                    </div>
                    <div>
                      <p className="font-light">Expire le</p>
                      <p
                        id="imageExpDate"
                        className="font-medium tracking-wider h-6 w-14"
                      >
                        {cardDate ? cardDate : "12/07"}
                      </p>
                    </div>
                  </div>
                </div>
